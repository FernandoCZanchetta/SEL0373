...
#define SoftwareSerial_h //Macro definida para o uso de UART
...
EXPORT errorr_t uart_writeN(uart_connection_t conn, buffer_view_t buffer) {
  return conn.uart->write(buffer.data, buffer.size);
}

EXPORT errorr_t uart_readN(uart_connection_t conn, buffer_view_t buffer) {
