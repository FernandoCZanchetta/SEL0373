 ...
 */
EXPORT errorr_t i2c_transmit(i2c_device_t device, buffer_view_t buffer) {
 ...
 * endereço de registrador
 */
EXPORT errorr_t i2c_receive(i2c_device_t device, buffer_view_t buffer) {
...
EXPORT errorr_t spi_transceive(spi_device_t device, buffer_view_t rx_buffer,
...
EXPORT errorr_t uart_transmit(uart_connection_t conn, buffer_view_t buffer) {
  LOG("Transmiting via UART%d: ", *conn.uart);
  BUFFER("Bytes", buffer);
  return 0;
}

EXPORT errorr_t uart_receive(uart_connection_t conn, buffer_view_t buffer) {
...
EXPORT errorr_t adc_init(adc_t *adc) {
