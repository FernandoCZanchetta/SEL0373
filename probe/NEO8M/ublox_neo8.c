...
static void ubx_checksum(int len, uint8_t * packet) {
	packet[len - 2] = 0x00;
	packet[len - 1] = 0x00;
 ...
 */
static errorr_t set_protocol(ublox_gps_t gps) {
 ...
 * ideal para sondas
 */
static errorr_t set_model_mode(ublox_gps_t gps) {
	static const  uint8_t message[] = { 0xB5, 0x62, 0x06, 0x24, 0x24, 0x00, 0xFF, 0xFF, 0x06,
			...
			0x16, 0xDC };
	buffer_view_t bv = { .data = (uint8_t *) message, .size = sizeof(message) };
	return gps_transmit(gps.conn, bv);
}
 ...
 */
static errorr_t trigger(ublox_gps_t gps) {
	static uint8_t message[] = { 0xB5, 0x62, // sync
...
static uint8_t get(ublox_gps_t gps){
	uint8_t x = 0;
	buffer_view_t data_v = {.data=&x, .size = sizeof(x)};
 ...
 */
errorr_t ublox_get(ublox_gps_t gps, ublox_pvt_t *pvt) {
	trigger(gps);
#if defined UBLOX_I2C
	uint8_t stream = UBLOX_STREAM_REG;
	errorr_t e = i2c_transmit(gps.conn, (buffer_view_t){.data=&stream, .size=1});
	if(e){
		return e;
	}
 ...
 */
errorr_t ublox_init(ublox_gps_t gps) {
	disableNmea(gps);
	set_protocol(gps);
	set_model_mode(gps);
	return 0;
